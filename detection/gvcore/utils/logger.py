...
class Logger:
    ...
    def setup_logger(self, cfg, is_main_process):
        if wandb_installed:
...
class GenericMetric:
    def __init__(self, window_size=20, log_interval=1, distributed=False, printable=True):
        self.buffer = {}
        self.window_size = window_size
    ...
    def _collect(self, window_size=None):
        assert len(self.buffer) != 0, "Nothing in the buffer."
...
class GenericSummary:
    ...
    def add_metrics(self, window_size=None, log_interval=None, namespace="default", printable=True):
    ...
    def update(self, metrics, namespace="default"):
        if namespace not in self.metrics:
            self.add_metrics(namespace=namespace, printable=True)
