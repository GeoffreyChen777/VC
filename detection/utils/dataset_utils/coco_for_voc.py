...
with open("./utils/dataset_utils/instances_unlabeledtrainval20class.json", "r") as f:
    idx = json.load(f)


imgs = {}
for img in tqdm(idx["images"]):
...
labels = {}
max_cls = 0
for label in tqdm(idx["annotations"]):
    cls = cocometa.coco91to80[label["category_id"]]
    cls_name = coco2voc[cocometa(cls)["name"]]
    cls = vocmeta[cls_name]["id"]
    if cls > max_cls:
        max_cls = cls
    bbox = np.array(label["bbox"] + [cls], dtype=np.float32).reshape(1, -1)
    img_id = label["image_id"]
    if img_id not in labels:
        labels[img_id] = {"label": bbox, "image_path": "./data/COCO2017/images/" + imgs[img_id]}
    else:
        labels[img_id]["label"] = np.concatenate((labels[img_id]["label"], bbox), axis=0)

voc_cache = torch.load("./cache/voc12_trainval.dict")
labels.update(voc_cache)

torch.save(labels, "./cache/coco17_train_voccls_and_voc12_trainval.dict")
