...
try:
    from fastevalapi import _C
except:
    import logging

    logger = logging.getLogger()
    logger.error("Please run cd ext && python setup.py install first.")
...
class COCOevalFast(COCOeval):
    ...
    def evaluate(self):
        ...
        toc = time.time()
        logger.info("COCOevalFast.evaluate() finished in {:0.2f} seconds.".format(toc - tic))
    ...
    def accumulate(self):
        ...
        toc = time.time()
        logger.info("COCOevalFast.accumulate() finished in {:0.2f} seconds.".format(toc - tic))
